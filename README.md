...
## 📝 理论与数据参考
在真实生产线中，缺陷样本极少出现，通常只能依靠仅含 `good` 良品的训练集。MVTec 3D-AD 的 `test/xxx/xyz` 使用 `.tiff` 文件来存放高精度的像素级三维空间信息截面。本项目通过挖掘这类**完全无监督**场景的核心难点，成功证实了联合 3D 法向量分析相比纯二维投影提取拥有跨越维度的强力表现。二维投影提取拥有跨越维度的强力捕捉能力。
